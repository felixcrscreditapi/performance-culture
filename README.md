...
document `CRS's Performance Culture`. **“Story Night” art direction** — a deep midnight sky, a living
twinkling **starfield** (shooting stars + constellations), moonlight, and the journey told as a long
philosophers' walk under the stars (Socrates-and-Alexander vibe): moonlit hills, a winding path, two
figures in conversation, ending at **dawn**. Coloured after [crscreditapi.com](https://crscreditapi.com)
with a CRS **electric-violet** signal colour (`#9D6BFF`); grotesk-display + monospace type.
...
A full-page **twinkling starfield** (canvas: shooting stars + faint constellation links = the “network
of peers”), soft moonlight glows, parallax on the hero/interlude/movement night-scenes, full-bleed
image chapters, a marquee ticker, a fixed index rail, masked line reveals, a text-decode hero, custom
cursor. Respects `prefers-reduced-motion` (static starfield, no shooting stars); append `?static=1` to
the URL for a calm, animation-free render. The cinematic scenes are regenerated by
